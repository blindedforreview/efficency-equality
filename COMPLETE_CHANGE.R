...
#### Data cleaning ####
library(Hmisc)
lag.maker <- function(d = math, variables = c("gini", "icc", "ach", "p5.p95", "p15.p85"), lag = 1, group = "cnt")
{
...
{
	#gini
	gini.lag.results<- lm(ach ~ gini.change.1*cnt + offset(ach.lag.1*1) + as.factor(year), data = domain)
	#ICC
	icc.lag.results <- lm(ach ~ icc.change.1*cnt + offset(ach.lag.1*1) + as.factor(year), data = domain)
	#p5.p95 
	p95.lag.results <- lm(ach ~ p5.p95.change.1*cnt + offset(ach.lag.1*1) + as.factor(year), data = domain)
	#return results
	return.results <- list(gini.results = gini.lag.results, icc.results = icc.lag.results,
						   p95.results = p95.lag.results)
}
...
##### Model Summaries #####
# Note that lm struggles with some of the P95 - P5 models due scaling issues
# Parameters derived from deviding P95 - P5 by 500 give identical results 
# to two decimal places when rescaled to original matrix. 

recurse <- function (L, f)
{
	if (inherits(L, "lm")) f(L)
...
####MATH Results ####
tmp <- fixed.effects.results$math$gini.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$math$gini.results[33:59,1])
median(tmp2)
tmp <- fixed.effects.results$math$icc.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$math$icc.results[33:59,1])
median(tmp2)
tmp <- fixed.effects.results$math$p95.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$math$p95.results[33:59,1])
median(tmp2)

####READING Results ####
tmp <- fixed.effects.results$read$gini.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$read$gini.results[33:59,1])
median(tmp2)
tmp <- fixed.effects.results$read$icc.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$read$icc.results[33:59,1])
median(tmp2)
tmp <- fixed.effects.results$read$p95.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$read$p95.results[33:59,1])
median(tmp2)

####SCIENCE Results ####
tmp <- fixed.effects.results$scie$gini.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$scie$gini.results[33:59,1])
median(tmp2)
tmp <- fixed.effects.results$scie$icc.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$scie$icc.results[33:59,1])
median(tmp2)
tmp <- fixed.effects.results$scie$p95.results[2,1]
tmp2 <- c(tmp, tmp + fixed.effects.results$scie$p95.results[33:59,1])
median(tmp2)
